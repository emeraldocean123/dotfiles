...
if (-not $isOpenCode) {
    # Centralize modules: include PowerShell (PS7) user modules in Windows PowerShell 5.1
    $ps7UserModules = Join-Path $HOME 'Documents/PowerShell/Modules'
    if (Test-Path $ps7UserModules) {
        $paths = $env:PSModulePath -split ';'
        if ($paths -notcontains $ps7UserModules) {
            $env:PSModulePath = "$ps7UserModules;" + $env:PSModulePath
        }
    }
...
# 3) Oh My Posh prompt (skip in opencode)
if (-not $isOpenCode) {
